...
```{r}
population <- resp_body_json(response, simplifyDataFrame = FALSE, simplifyMatrix = TRUE)
```
...
library(tidyverse)
library(janitor)
population_raw <- resp_body_json(response, simplifyVector = FALSE)
population <- as.data.frame(do.call(rbind, population_raw), stringsAsFactors = FALSE) |> as_tibble() |> row_to_names(1) |> clean_names() |> select(-state) |> rename(state_name = name) |> pivot_longer(starts_with("pop_"), names_to = "year", values_to = "population") |> mutate(year = readr::parse_number(year), population = as.numeric(population)) |> mutate(state = case_when(state_name == "District of Columbia" ~ "DC", state_name == "Puerto Rico" ~ "PR", TRUE ~ state.abb[match(state_name, state.name)])) |> select(state, state_name, year, population)
  # convert to tibble
  # remove stat column
...
population |>
  ...
  coord_flip() +
  facet_wrap(~year) +
  labs(title = "States' population (2020 and 2021)", x = "State", y = "Population") +
...
regs_raw <- jsonlite::fromJSON(url, simplifyVector = FALSE)
regions <- purrr::imap_dfr(regs_raw, function(el, nm) {
  rn <- if (!is.null(el$region_name)) el$region_name else if (!is.null(el$name)) el$name else nm
  rg <- if (!is.null(el$region)) el$region else nm
  tibble::tibble(
    state_name = unlist(el$states, use.names = FALSE),
    region = rg,
    region_name = rn
  )
})
...
```{r}
population <- population |> mutate(state_name = unlist(state_name, use.names = FALSE)) |> left_join(regions, by = "state_name")
```
...
cases_raw <- request(api) |> req_url_query(`$limit` = 50000) |> req_perform() |> resp_body_json(simplifyVector = TRUE) |> tibble::as_tibble()
```

We see exactly 1,000 rows. We should be seeing over $52 \times 3$ rows per state.

**Comment:** No—this isn’t the full dataset. Only 1,000 rows are shown because the CDC’s default row limit prevents retrieving more.
...
```{r}
api <- "https://data.cdc.gov/resource/pwn4-m3yp.json"
cases_raw <- request(api) |> req_url_query(`$limit` = 10000000000) |> req_perform() |> resp_body_json(simplifyVector = TRUE) |> tibble::as_tibble()
cases <- cases_raw |> transmute(state, date = as.Date(substr(end_date, 1, 10)), cases = as.numeric(new_cases))
```
...
cases |>
  ...
  inner_join(population |> filter(year %in% c(2020, 2021)) |> select(state, year, population, region_name), by = c("state", "year")) |>
  filter(!is.na(region_name), !is.na(population), population > 0) |>
  mutate(cases_per_100k = (cases / population) * 100000) |>
  ggplot(aes(x = date, y = cases_per_100k, group = state, color = state)) +
  geom_line(alpha = 0.5, linewidth = 0.3, na.rm = TRUE) +
  facet_wrap(~ region_name, labeller = label_wrap_gen(width = 30)) +
  labs(title = "COVID-19 Cases per 100,000 (2020–2021)", x = "Date", y = "Cases per 100,000") +
  scale_x_date(date_breaks = "3 months", date_labels = "%Y-%m", expand = c(0.01, 0)) +
  ...
  guides(color = guide_legend(nrow = 2, byrow = TRUE, override.aes = list(linewidth = 0.6, alpha = 0.8)))
```
...
```{r}
deaths_raw <- request(deaths_url) |> req_url_query(`$select` = "state,end_date,covid_19_deaths", `$where` = "sex = 'All Sexes' AND age_group = 'All Ages'", `$limit` = 10000000000) |> req_perform() |> resp_body_json(simplifyVector = TRUE) |> tibble::as_tibble()
deaths <- deaths_raw |> transmute(state_name = state, date = lubridate::ymd(substr(end_date, 1, 10)), deaths = as.numeric(covid_19_deaths)) |> mutate(state = dplyr::case_when(state_name == "District of Columbia" ~ "DC", state_name == "Puerto Rico" ~ "PR", TRUE ~ state.abb[match(state_name, state.name)])) |> select(state, date, deaths)
```
