...
```{r}
reorder_within <- function(x, by, within, fun=mean, sep="___") stats::reorder(paste(x, within, sep=sep), by, fun)
scale_y_reordered <- function(sep="___") ggplot2::scale_y_discrete(labels=function(x) gsub(paste0(sep, ".*$"), "", x))

population |>
  mutate(state_name=reorder_within(state_name, population, year)) |>
  ggplot(aes(y=state_name, x=population)) +
  geom_col(orientation="y") +
  facet_wrap(~year, ncol=1, scales="free_y") +
  scale_y_reordered() +
  scale_x_continuous(labels=function(x) format(x, big.mark=",")) +
  labs(x="Population", y=NULL, title="State Population: 2020 vs 2021") +
  theme(axis.text.y=element_text(size=3), panel.grid.major.y=element_blank())
```
...
library(purrr)
url <- "https://github.com/datasciencelabs/2025/raw/refs/heads/main/data/regions.json"
regions_list <- jsonlite::read_json(url, simplifyVector = FALSE)
regions <- purrr::map_df(regions_list, \(x) tibble(state_name = unlist(x$states), region = as.integer(x$region[[1]]), region_name = x$region_name)) |> mutate(region_name = dplyr::if_else(region_name == "New York and New Jersey, Puerto Rico, Virgin Islands", "NY & NJ, PR, VI", region_name))
```
...
cases |>
  ...
  left_join(population |> filter(year %in% c(2020, 2021)) |> select(state, year, population, region_name), by = c("state", "year")) |>
  ...
  labs(title = "Daily COVID-19 cases per 100,000 by state (2020–2021)",
